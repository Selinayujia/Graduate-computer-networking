...
func flipMap(serverHashMap *map[string][]string, hashServerMap *map[string]string) {
	for serverName, hashList := range *serverHashMap {
		for _, hash := range hashList {
			(*hashServerMap)[hash] = serverName
		}
	}
}
...
func uploadToServer(fileMetaData *FileMetaData, client *RPCClient) (e error) {
	...
	if errors.Is(osErr, os.ErrNotExist) {
		...
		hashServerMap := make(map[string]string)
		client.GetBlockStoreMap(fileMetaData.BlockHashList, &blockStoreMap)
		flipMap(&blockStoreMap, &hashServerMap)

		for i := 0; i < numBlocks; {
			bytes := make([]byte, client.BlockSize)
			leng, readErr := file.Read(bytes)
			if readErr != nil && readErr != io.EOF {
				log.Println("Error Reading Bytes From File: ", readErr)
			}
			bytes = bytes[:leng]
			block := Block{BlockData: bytes, BlockSize: int32(leng)}

			hash := GetBlockHashString(bytes)

			serverAddr := hashServerMap[hash][len("blockstore"):]
			var success bool
			blockUploadErr := client.PutBlock(&block, serverAddr, &success)
			if !success {
...
func updateServerWithLocal(updatedLocalMeta map[string]*FileMetaData, remoteMeta map[string]*FileMetaData, client *RPCClient) {
	...
	for fileName, metaData := range updatedLocalMeta {
		if remoteMetaData, ok := remoteMeta[fileName]; ok {
			if metaData.Version > remoteMetaData.Version {
				...
				uploadErr := uploadToServer(metaData, client)
				if uploadErr != nil {
...
func downloadFromServer(remoteFileMetaData *FileMetaData, client *RPCClient) (e error) {
	baseDir, _ := filepath.Abs(client.BaseDir)
	...
	blockStoreMap := make(map[string][]string)
	hashServerMap := make(map[string]string)
	client.GetBlockStoreMap(remoteFileMetaData.BlockHashList, &blockStoreMap)
	flipMap(&blockStoreMap, &hashServerMap)

	for _, hash := range remoteFileMetaData.BlockHashList {

		var block Block
		serverName := hashServerMap[hash]
		serverAddr := serverName[len("blockstore"):]
		blockErr := client.GetBlock(hash, serverAddr, &block)
		if blockErr != nil {
...
func updateLocalWithServer(updatedLocalMeta map[string]*FileMetaData, remoteMeta map[string]*FileMetaData, client *RPCClient) {
		...
		if ok {
			if localMetaData.Version < metaData.Version {
				*localMetaData = *metaData
				_ = downloadFromServer(metaData, client)
			...
			*updatedLocalMeta[fileName] = *metaData
			downloadFromServer(metaData, client)
		}
	}
}
...
func ClientSync(client RPCClient) {
	...
	//fmt.Println("before", localFileMetaData)
	updateServerWithLocal(localFileMetaData, remoteMeta, &client) // upload

	updateLocalWithServer(localFileMetaData, remoteMeta, &client) // download
	//fmt.Println("after", localFileMetaData)
	// write to db
